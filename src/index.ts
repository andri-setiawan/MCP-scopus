...
function formatAuthor(author: any) {
  ...
  const name = surname && givenName ? `${surname}, ${givenName}` : (author["dc:title"] || "");

  return {
    scopusAuthorId: author["dc:identifier"]?.replace("AUTHOR_ID:", "") || "",
    name,
    givenName,
    surname,
    ...
    documentCount: author["document-count"] || "0",
    citationCount: author["cited-by-count"] || "0",
    hIndex: author["h-index"] || "0",
